...
// Insert “extras” UI into the first pillar (physical) once state exists
...
{pillarsWithExtras.map((p, i) => (
<div key={p.title} className="shadow-lg rounded-2xl p-4 space-y-3 border">
...
<p className="text-xs text-gray-600">{p.example}</p>
...
className="w-full min-h-[90px] rounded border px-2 py-2 text-sm"
/>
</div>
))}
</div>
